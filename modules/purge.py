...
# merge sample data: Match Experiments and Peak Names
    ...
    purge = purge.rename(columns={'Rel.Area_y': sample2, 'Rel.Area_x': 'isolated' })
...
def getRegression(IPCsample, imp, data, CRITexp, uncrit_only=False, verbose=False, inverse=False): 

    regParams = pd.DataFrame()

    if (uncrit_only):
        data = data[~data.index.get_level_values('Experiment').isin(CRITexp)]
        
    corrLevels = getPurge('cryst', IPCsample, data, inv=False)
    
    df = corrLevels
    df = df[df['Peak Name'].isin([imp])]
    ...
    try:
        ...
        current = pd.Series([IPCsample, imp, slope, intercept, r, p, std_err])

        regParams = regParams.append(current, ignore_index=True)
        regParams.columns = ['Sample Type', 'Peak Name', 'slope', 'intercept', 'r', 'p', 'std_err']  
        # print(regParams)  
        ...
        if verbose == True:
            print('Insufficient data for regression of ', IPCsample, ' /  ', imp)





def getPurityModel(samples, imps, data, CRITexp, uncrit_only=False, verbose=False, inverse=False):
    # selSAMPLES = SAMPLES
    
    purityModel = pd.DataFrame(columns = ['Sample Type', 'Peak Name', 'slope', 'intercept', 'r', 'p', 'std_err'])

    for s in samples:
        for i in imps:
            rg = getRegression(s, i, data, CRITexp, uncrit_only=uncrit_only, verbose=verbose, inverse=inverse)
            purityModel = pd.concat([purityModel, rg])
   
    purityModel = purityModel.dropna(axis=0, how='any')

    print('inverse regression: ', inverse)
    return purityModel
